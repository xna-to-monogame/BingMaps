...
public class BingMapsTiles : IDisposable
{
    ...
    public BingMapsTiles(GameServiceContainer services, Texture2D unavailableImage, Vector2 tileDimensions, int planeDimensions)
    {
        _services = services;
        ...
        if (_apiService == null)
        {
            throw new ArgumentNullException(nameof(services), $"Services does not contain an instance of {nameof(BingApiService)}");
        }

        _graphicsDeviceService = services.GetService<IGraphicsDeviceService>();
        if (_apiService == null)
        {
            throw new ArgumentNullException(nameof(services), $"Services does not contain an instance of {nameof(IGraphicsDeviceService)}");
        }
    ...
    public async Task InitializeActiveTilePlane(GeoCoordinate centerGeocoordinate)
    {
        ...
        else
        {
            ...
            await GetActivePlaneImagesAsync(centerGeocoordinate);
        }
    }
    ...
    private async Task GetActivePlaneImagesAsync(GeoCoordinate centerCoordinate)
    {
        ...
        for (int x = 0; x < ActiveTilePlaneSize; x++)
        {
            ...
            for (int y = 0; y < ActiveTilePlaneSize; y++)
            {
                ...
                if(cellInformation == null)
                {
                    cellInformation = new TileInformation(_services);
                ...
                try
                {
                    ...
                    Texture2D? image= await _apiService.GetImageFromServer(tileCenterGeoCoordiante,_tileDimensions, _zoomLevel, ViewType);
                    cellInformation.UnloadImage();
    ...
    private async Task MoveToNextPhaseAsync()
    {
        ...
        {
            await GetActivePlaneImagesAsync(_centerGeoCoordinate);
        }
    }

...
internal class BingMapsViewer
{
    ...
    public BingMapsViewer(GameServiceContainer services, Texture2D defaultImage, Texture2D unavailableImage, GeoCoordinate location, int zoomLevel, int gridSize)
    {
        _apiService = services.GetService<BingApiService>();
        if (_apiService == null)
        {
            throw new ArgumentException($"Servies do not contain an instance of {nameof(BingApiService)}", nameof(services));
        }

        _graphicsDeviceService = services.GetService<IGraphicsDeviceService>();
        if (_graphicsDeviceService == null)
        {
            throw new ArgumentException($"Services do not contain an instance of {nameof(IGraphicsDeviceService)}", nameof(services));
        }
        ...
        ZoomLevel = zoomLevel;
        ViewType = BingMapsViewType.Arial;

        _screenBounds = _graphicsDeviceService.GraphicsDevice.Viewport.Bounds;
        ...
        ActiveTiles = new BingMapsTiles(_services, _unavailableImage, _tileDimensions, gridSize)
        {
            ZoomLevel = zoomLevel,
            ViewType = ViewType
        };
    ...
    public void Draw(SpriteBatch spriteBatch)
    {
        ...
        for (int i = 0; i < ActiveTiles.ActiveTilePlaneSize; i++)
        {
            for (int j = 0; j < ActiveTiles.ActiveTilePlaneSize; j++)
            {
                ...
                spriteBatch.Draw(image, _screenCenterVector + _offset + extraOffset, null, Color.White, 0, _tileDimensions / 2.0f, 1.0f, SpriteEffects.None, 0.0f);
